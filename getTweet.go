...
func Handler(request Request) ([]Response, error) {
    ...
    // 取得したデータ1件1件に対して、必要な情報のみ絞り込み
    for _, data := range datas {
        // 取得したJSONの開業をスペースに置き換え
        str := []byte(strings.Replace(data.TweetData, "\n", " ", -1))

        // JSONを変数に変換
        err := json.Unmarshal(str, &tweet)
        if err != nil {
            spew.Dump(tweet)
            fmt.Println(err)
        }

        // レスポンス用にツイートのIDとユーザーのIDのみ格納
        row := Response {
            Id: tweet.IDStr,
